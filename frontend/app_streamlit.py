...
with st.form("patient_form"):
    ...
    with col1:
        ...
        preexisting_conditions = st.selectbox(
            ...
            ["Asthma", "Diabetes", "Hypertension", "Obesity", "Cardiovascular"]
        )
        covid_strain = st.selectbox(
            "COVID Strain", 
            ["Alpha", "Beta", "Delta", "Omicron", "XBB.1.5"]
        )
        symptoms = st.selectbox("Symptoms", ["Mild", "Moderate", "Severe"])
    ...
    with col3:
        long_covid_symptoms = st.selectbox(
            "Long COVID Symptoms", 
            ["None", "Fatigue", "Brain Fog", "Shortness of Breath", "Chest Pain"]
        )
        occupation = st.selectbox(
            "Occupation", 
            ["Healthcare", "Teacher", "Office Worker", "Driver", "Student", "Unemployed"]
        )
    ...
    with col4:
        ...
        recovery_classification = st.selectbox(
            ...
            ["Fast Recovery", "Typical Recovery", "Delayed Recovery"]
        )
...
if submit_button:
    ...
    if result:
        ...
        with res_col2:
            ...
            st.markdown("---")
            st.markdown("""
            **Disclaimer**: This prediction is based on statistical modeling and should not replace 
            professional medical advice. Please consult healthcare professionals for personalized guidance.
            """)
...
try:
    ...
    if response.status_code == 200:
        ...
        st.sidebar.success(f"✅ Prediction Service: {status_data['prediction_service']}")
        
        if status_data['explanation_service'] == 'available':
            st.sidebar.success(f"✅ RAG Analysis: Available")
        else:
            st.sidebar.warning(f"⚠️ RAG Analysis: Limited")
    else:
